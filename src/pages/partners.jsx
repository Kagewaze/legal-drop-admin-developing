    ...
    <section className="p-4 space-y-4">
      ...
      {detail ? (
        <section className="border p-4 space-y-3">
          ...
          {payoutSummary ? <><p>Readiness: {payoutSummary.destination?.readinessState || 'NO_DESTINATION'} · Destination: {payoutSummary.destination?.maskedSummary || 'Not configured'} · Requirements: {payoutSummary.destination?.requirementsDue?.join(', ') || 'None reported'}</p><p>Release policy: {payoutSummary.releasePolicy.activated ? 'ACTIVE' : 'NOT ACTIVATED'}</p><p>Payout attention: {payoutSummary.signals?.join(', ') || 'None'}</p>{payoutSummary.balances.map(b=><p key={b.currency}>{b.currency}: held {b.heldMinor} · available {b.availableMinor} · reserved {b.reservedMinor} · transferred {b.transferredMinor}</p>)}</> : <p>No payout configuration.</p>}
